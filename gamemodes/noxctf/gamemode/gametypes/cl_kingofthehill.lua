	...
	if nextholdingupdate < curtime then
		...
		if kothent and kothent:IsValid() then
			...
			for _, teamid in pairs(TEAMS_PLAYING) do
				team.SetCurrentInArea(teamid, 0)
			end
			...
			for _, ent in pairs(player.GetAll()) do
				if ent:Alive() then
					...
					if pos:Distance(kpos) < 512 then
						...
						team.SetCurrentInArea(teamid, team.GetCurrentInArea(teamid) + 1)
					end
				end
			end
			...
			for teamid, _ in pairs(team.GetAllTeams()) do
				if topplayers < team.GetCurrentInArea(teamid) then
					topplayers = team.GetCurrentInArea(teamid)
				end
			end
	...
	for i, teamid in pairs(TEAMS_PLAYING) do
		...
		if teamid == currentholding then
			...
			if 0 < math.sin(RealTime() * 23) then
				...
				draw.SimpleText(team.GetCurrentInArea(teamid), "teamplay", fx + 60, fy + 16, col, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
			else
				surface.SetDrawColor(255, 255, 255, 255)
				surface.DrawOutlinedRect(fx, fy, 64, 32)
				draw.SimpleText(text, "DefaultBold", fx + 24, fy + 16, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

				draw.SimpleText(team.GetCurrentInArea(teamid), "teamplay", fx + 60, fy + 16, color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
			end
			if tim < 20 then
