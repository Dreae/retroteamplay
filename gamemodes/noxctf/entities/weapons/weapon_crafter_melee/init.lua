...
local function SwingCallback(attacker, trace, dmginfo)
	...
	if trace.Hit and trace.HitPos:Distance(trace.StartPos) <= 70 then
		...
		attacker:EmitSound("weapons/melee/wrench/wrench_hit-0"..math.random(4)..".wav", 75, 100)
	else
	...
	if ent:IsValid() and trace.HitPos:Distance(trace.StartPos) <= 70 then
		...
		if ent.ScriptVehicle and ent:Team() == attacker:Team() then
			...
			local newhealth = math.min(ent:GetVHealth() + 15 * damageMult, maxhealth)
		...
		elseif ent.PHealth and ent:GetTeamID() == attacker:Team() then
			...
			elseif ent.Destroyed then
				if not ent:IsPlayerHolding() then
					local newhealth = math.ceil(math.min(ent.PHealth + math.ceil((OVERTIME and 10 or 30) * damageMult), maxhealth))
		...
		elseif ent.TakeSpecialDamage then
			if ent.PHealth then
				ent:TakeSpecialDamage(math.ceil(20 * damageMult), DMGTYPE_IMPACT, attacker)
			else
				local guarded = false
				...
				if not guarded then
					ent:TakeSpecialDamage(math.ceil(5 * damageMult), DMGTYPE_IMPACT, attacker)
				end
			end
		else
			ent:TakeDamage(ent.PHealth and math.ceil(20 * damageMult) or math.ceil(10 * damageMult), attacker)
		end
	end
...
function SWEP:PrimaryAttack()
	...
	for _, ent in pairs(ents.FindByClass("netherbomb")) do
		if ent:GetTeamID() ~= self.Owner:GetTeamID() and ent:GetPos():Distance(self.Owner:GetCenter()) <= 60 then
			ent:Remove()
		end
	end
end
...
function SWEP:SecondaryAttack()
	...
	for _, ent in pairs(ents.FindByClass("projectile_wrench")) do
		if ent.OriginalOwner == owner then return end
	end
	if CurTime() < self:GetNextPrimaryFire() then return end
	...
	owner:EmitSound("weapons/slam/throw.wav", 80, math.Rand(95, 105))
	local ent = ents.Create("projectile_wrench")
	if ent:IsValid() then
		...
		ent:Spawn()
		ent:SetAngles(Angle(owner:EyeAngles().pitch ,owner:EyeAngles().yaw,90))
		local phys = ent:GetPhysicsObject()
		if phys:IsValid() then
			phys:ApplyForceCenter(owner:GetAimVector() * 1400)
			phys:AddAngleVelocity(Vector(0,-2000,0))
		end
	end
	owner:CustomGesture(ACT_HL2MP_GESTURE_RANGE_ATTACK_GRENADE)
end
