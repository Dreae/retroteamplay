...
function EFFECT:Render()
	if 0 < EFFECT_QUALITY and not self.Owner:IsValid() then return end
	...
	for i=1, ent:GetPhysicsObjectCount() do
		...
		if phys and phys:IsValid() then
			...
			if math.random(1, 2) == 1 then
				...
				if 1 < EFFECT_QUALITY then
					for m=1, math.random(2, EFFECT_QUALITY) do
						local bpos = pos
						local normal = VectorRand()
						local bpos2 = pos + normal * 4
						local xmax = math.random(2, EFFECT_QUALITY)
						for x=1, xmax do
							if x == xmax then
